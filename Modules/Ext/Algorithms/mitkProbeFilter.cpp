...
void mitk::ProbeFilter::GenerateInputRequestedRegion()
{
  ...
  timestep = inputTimeGeometry->TimePointToTimeStep( timeInMS );
  if( ( timeInMS > std::numeric_limits<ScalarType>::min() ) && ( inputTimeGeometry->IsValidTimeStep( timestep ) ) )
  ...
  timestep = inputTimeGeometry->TimePointToTimeStep( timeInMS );
  if ((timeInMS > std::numeric_limits<ScalarType>::min()) && (outputTimeGeometry->IsValidTimeStep(timestep)))
  ...
  timeInMS = outputTimeGeometry->TimeStepToTimePoint(outputRegion.GetIndex(3));
  timestep = sourceTimeGeometry->TimePointToTimeStep( timeInMS );
  if ((timeInMS > std::numeric_limits<ScalarType>::min()) && (sourceTimeGeometry->IsValidTimeStep(timestep)))
